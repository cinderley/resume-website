		...
		for(int row = map.length - 1; row >= 0; row --) {
			for(int col = map.length - 1; col >= 0; col --) {
				if(map[row][col] > highestProb && guesses[row][col] == '.') {
					...
					highestProb = tieBreaker(guesses, map, row, col, 10) < tieBreaker(guesses, map, highestProbRow, highestProbCol, 10)? map[row][col]: map[highestProbRow][highestProbCol];
				} 
			}
		}
